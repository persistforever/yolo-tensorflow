        ...
        for pageid in data:
            ...
            for text in texts:
                if text['type'] == 2:
                    ...
                    n_passed = 0
                    for w in ws:
                        left = text['position'][0] + int(n_passed * uint)
                        right = text['position'][0] + int((n_passed+len(w)) * uint)
                        top = text['position'][2]
                        bottom = text['position'][3]
                        n_passed += len(w)

                        if w in word_dict:
                            color = [int(255 * t) for t in word_dict[w]]
                        else:
                            color = [150, 150, 150]
                
                        words.append({'text': w, 'position': [left, right, top, bottom], 'color': color, 'type': 2})
            
            contents_dict[docid][pageid] = {
                'words': words, 'texts': texts, 'size': size, 
                'tables': tables, 'others': others, 'curves': curves}
...
if 'Windows' in platform.platform():
    ...
    draw_image(contents_dict, '/home/caory/github/darknet-master/datasets/table-v3')
    # create_labels(contents_dict, '/home/caory/github/darknet-master/datasets/table-v2/')
